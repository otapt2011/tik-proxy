...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Sort by diggCount (most liked)
    const sorted = [...items].sort((a, b) => b.stats.diggCount - a.stats.diggCount);
    ...
    console.error('Top liked videos API error:', err);
    res.status(500).json({ error: err.message || 'Failed to fetch top liked videos' });
  }
}