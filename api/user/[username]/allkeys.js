...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (path) {
      // First, try splitting by dots and traversing
      const keys = path.split('.');
      for (const jsonObj of allJsonData) {
        const found = getValueByKeyArray(jsonObj, keys);
        if (found !== null) {
          targetObject = found;
          targetSource = `${path} (dot-traversed)`;
          break;
        }
      }
      // If not found, try the whole path as a single key (for keys containing dots)
      if (!targetObject) {
        for (const jsonObj of allJsonData) {
          if (jsonObj && typeof jsonObj === 'object' && jsonObj[path] !== undefined) {
            targetObject = jsonObj[path];
            targetSource = `${path} (single key)`;
            break;
          }
        }
      }
    ...
    if (!targetObject) {
      ...
      return res.status(404).json({
        ...
        hint: 'Try using the exact key name (may contain dots)'
      });
    }
