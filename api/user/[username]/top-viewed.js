...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Sort by playCount (most viewed)
    const sorted = [...items].sort((a, b) => b.stats.playCount - a.stats.playCount);
    ...
    console.error('Top viewed videos API error:', err);
    res.status(500).json({ error: err.message || 'Failed to fetch top viewed videos' });
  }
}