...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Use secUid to call TikTok's internal API with pagination
    ...
    const nextCursor = data.cursor;  // TikTok returns the cursor for next page
    ...
    res.status(200).json({
      ...
      cursor: parseInt(cursor),        // current cursor used
      nextCursor: nextCursor,          // cursor for next request
      hasMore: hasMore,                // whether more pages exist
